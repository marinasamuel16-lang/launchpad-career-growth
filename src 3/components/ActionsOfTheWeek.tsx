...
/**
 * `onLevelUp` is accepted but unused — XP was removed from this card, and
 * keeping the prop means profile.tsx needs no edit.
 */
export function ActionsOfTheWeek(_props: { onLevelUp?: (level: number) => void } = {}) {
  const themeQuery = useActiveTheme();
  const { isDone, setDone } = useDoneActions();
          ...
          {actions.map((a) => {
            ...
            return (
              ...
              >
                ...
                {done && <Check className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />}
              </label>
            );
          })}
