...
/** career_memory_entries.title is CHECK (length BETWEEN 1 AND 120). */
...
/**
 * Actions run longer than a Career Memory title is allowed to be, so trim at a
 * word boundary and keep the full text in the description. Silently dropping
 * the entry — which is what an unhandled constraint violation does — is the one
 * outcome that isn't acceptable here.
 */
function toMemoryTitle(text: string): { title: string; description: string | null } {
  const clean = text.trim();
  if (clean.length <= MEMORY_TITLE_MAX) return { title: clean, description: null };

  const cut = clean.slice(0, MEMORY_TITLE_MAX - 1);
  const lastSpace = cut.lastIndexOf(" ");
  const head = (lastSpace > 60 ? cut.slice(0, lastSpace) : cut).replace(/[,;:.\s]+$/, "");
  return { title: `${head}…`, description: clean };
}
...
const FALLBACKS: Record<Category, { action: string; why: string; minutes: number }[]> = {
  ...
  project: [
    { action: "Send your manager a three-line status update on your main project, unprompted.", why: "Visibility on work in progress beats a reveal at the end.", minutes: 10 },
  ...
  skill: [
    { action: "Name the skill standing between you and your next role, and book 30 minutes to practise it.", why: "A skill gap you have not named cannot be closed.", minutes: 10 },
    { action: "Ask someone good at the thing you want to learn how they got good at it.", why: "The shortcut is usually in the answer.", minutes: 10 },
  ],
  feedback: [
    { action: "Ask your manager what would move you from good to excellent in your next 1:1.", why: "Most managers only give the real answer when asked directly.", minutes: 5 },
  ...
  networking: [
    { action: "Message one person in a role you want to learn about, and ask for 15 minutes.", why: "People two steps ahead remember what your step felt like.", minutes: 10 },
  ...
  challenge: [
    { action: "Name one thing you have been avoiding, and write the first sentence you would use to raise it.", why: "The sentence is usually the hard part. The conversation follows it.", minutes: 10 },
    { action: "Identify your one recurring frustration this week and write down what would actually fix it.", why: "Frustration you can describe precisely becomes a proposal.", minutes: 15 },
  ],
  decision: [
    { action: "Write one sentence on where you want to be in two years, then see if you believe it.", why: "Goals you cannot say plainly are usually someone else's.", minutes: 10 },
    { action: "List what would have to be true for your current role to still be right in a year.", why: "Deciding to stay is a decision too — it deserves the same thinking.", minutes: 15 },
  ],
};
...
export const getTodayDashboard = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }): Promise<TodayDashboard> => {
    ...
    const [
      ...
      db.from("weekly_themes").select("title").eq("is_active", true).maybeSingle(),
    ]);
  ...
  .handler(async ({ data, context }): Promise<TodaysMove> => {
    ...
    const [
      ...
      db
        ...
        .limit(3),
      // What they've actually raised with the coach. Their own words only —
      // feeding the coach's replies back in would have the model building on
      // its own advice instead of on the person's situation.
      db
        .from("coach_messages")
    ...
    if (key) {
      try {
        ...
        // Oldest first, so the model reads it as a conversation rather than a list.
        const coachLines = [...(coachTurns ?? [])]
          .reverse()
...
WHAT THEY HAVE RAISED WITH THEIR COACH (their own words, most recent last)
${coachLines || "(they haven't talked to the coach yet)"}
...
- ONE sentence, UNDER 120 CHARACTERS. Tight and specific beats thorough.
- Second person, plain language, no preamble.
- If they raised something specific with their coach, prefer an action that moves
  THAT forward — named people, named projects, the thing they said they were stuck on.
  Their own words are the best signal you have. Use them without quoting them back.
...
Good: "Ask your manager what would move you from good to excellent on the Nexus project."
Bad: "Work on your visibility." / "Be more proactive." / Anything over two lines.
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a practical career coach writing one small daily action. Keep it under 120 characters. Respond with valid JSON only. Never invent facts about the person's employer, salary or performance.",
              },
 ...
 *
 * `recorded` tells the caller whether the Career Memory entry was actually
 * written. The completion itself still stands either way, but the UI must not
 * claim something was saved when it wasn't.
 */
export const completeMove = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (action.status === "completed") {
      return { ok: true as const, alreadyDone: true, recorded: true };
    }
    ...
    // Mirror into Career Memory. Never block completion on this, but do report it.
    const { title, description } = toMemoryTitle(action.action_text);
    ...
    if (memErr) console.error("[move] Career Memory mirror failed:", memErr.message);

    return { ok: true as const, alreadyDone: false, recorded: !memErr };
  });
