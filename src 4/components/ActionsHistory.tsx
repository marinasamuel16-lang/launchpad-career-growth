...
import { usePublishedThemes, useDoneActions } from "@/hooks/use-weekly-actions";
import { sbw, type WeeklyAction } from "@/integrations/supabase/weekly-types";

/**
 * Completed weekly actions, grouped by theme, shown as milestones the
 * user has earned inside their roadmap.
 */
export function ActionsHistory() {
  const themesQuery = usePublishedThemes();
  const { doneIds } = useDoneActions();
  ...
  const groups = useMemo(() => {
    ...
    return themes
      .map((t) => ({
        theme: t,
        actions: actions.filter((a) => a.theme_id === t.id && doneIds.has(a.id)),
      }))
